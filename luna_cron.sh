...
CRON_LINE="*/10 * * * * cd $SCRIPT_DIR && SSH_USERNAME=fignasi SSH_HOSTNAME=marcus.ace.chalmers.se SSH_KEY_PATH=/home/odisi/.ssh/luna_key $JULIA --project=. main.jl >> $LOG 2>&1 && git add luna.log && git commit -m 'cron: update luna.log' >> $LOG 2>&1"
